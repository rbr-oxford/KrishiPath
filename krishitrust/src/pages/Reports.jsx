...
const Reports = () => {
  ...
  // Agricultural Business & Finance Reports
  const reports = [
    { 
      id: 'weekly', 
  ...
  const getStatusColor = (status) => {
    switch(status) {
      case 'Generated': return 'bg-green-500/20 text-green-400 border-green-500/30';
    ...
    <div className="p-4 md:p-6 min-h-screen bg-[#020617]">
        ...
        {reportCategories.map((category) => (
          ...
          >
            ...
            <span className={`ml-1 px-1.5 py-0.5 rounded-full text-xs ${
              reportType === category.id ? 'bg-white/20' : 'bg-white/5'
            }`}>
              {category.count}
            </span>
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6">
        {filteredReports.map((report) => (
          <div 
            key={report.id} 
            className="group relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5 hover:border-green-500/30 transition-all duration-300 cursor-pointer hover:scale-[1.02] hover:shadow-2xl hover:shadow-green-500/5 overflow-hidden"
          >
            ...
            <div className="relative">
              ...
              <div className="mt-3 grid grid-cols-2 gap-1 p-2 bg-[#0d1117] rounded-lg border border-white/5">
                <div className="text-center">
                  <div className="text-white text-sm font-bold">{report.metrics.shipments}</div>
                  <div className="text-gray-500 text-[8px] uppercase">Shipments</div>
                </div>
                <div className="text-center">
                  <div className={`text-sm font-bold ${
                    report.metrics.handlingScore > 70 ? 'text-green-400' :
                    report.metrics.handlingScore > 50 ? 'text-yellow-400' :
                    'text-red-400'
                  }`}>
                    {report.metrics.handlingScore}/100
                  </div>
                  <div className="text-gray-500 text-[8px] uppercase">Handling Score</div>
                </div>
                <div className="text-center">
                  <div className="text-red-400 text-sm font-bold">{report.metrics.incidents}</div>
                  <div className="text-gray-500 text-[8px] uppercase">Events</div>
                </div>
                <div className="text-center">
                  <div className={`text-sm font-bold ${
                    report.metrics.readiness > 70 ? 'text-green-400' :
                    report.metrics.readiness > 50 ? 'text-yellow-400' :
                    'text-red-400'
                  }`}>
                    {report.metrics.readiness}
                  </div>
                  <div className="text-gray-500 text-[8px] uppercase">Readiness</div>
                </div>
              </div>
              
              <div className="mt-3 flex gap-2">
                <button className="flex-1 px-3 py-1.5 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 rounded-lg text-xs font-medium transition-all duration-300 border border-blue-500/20">
                  📄 View
                </button>
                <button className="flex-1 px-3 py-1.5 bg-white/5 hover:bg-white/10 text-gray-300 rounded-lg text-xs font-medium transition-all duration-300 border border-white/5">
                  ⬇️ Export
                </button>
              </div>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-2 bg-[#0d1117] rounded-lg border border-white/5">
              ...
              <div className="flex-1">
                <div className="text-white text-sm">Weekly Handling Performance Report</div>
                <div className="text-gray-400 text-xs">Generated 2 hours ago</div>
              </div>
            ...
            <div className="flex items-center gap-3 p-2 bg-[#0d1117] rounded-lg border border-white/5">
              <span className="text-xl">⏳</span>
              <div className="flex-1">
                <div className="text-white text-sm">Finance Readiness Profile Report</div>
                <div className="text-gray-400 text-xs">Processing - 65% complete</div>
              </div>
            ...
            <div className="flex items-center gap-3 p-2 bg-[#0d1117] rounded-lg border border-white/5">
              <span className="text-xl">📅</span>
              <div className="flex-1">
                <div className="text-white text-sm">Predictive Readiness Forecast</div>
                <div className="text-gray-400 text-xs">Scheduled for Aug 12, 2026</div>
              </div>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-3">
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white text-sm">GPS Data</span>
          </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white text-sm">Environmental Sensors</span>
          </div>
          <span className="text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded-full">Online</span>
        </div>
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-3 flex items-center justify-between">
