        ...
        {Object.entries(sensorData).map(([key, data]) => (
          <div key={key} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-all duration-300 hover:scale-105">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`text-xs px-2 py-1 rounded-full ${data.status === 'Connected' || data.status === 'Active' || data.status === 'Online' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                {data.status}
              </span>
            </div>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs"><span className="text-gray-400">Vibration Level</span><span className="text-yellow-400">Moderate (1.8G)</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-400">Shock Events</span><span className="text-red-400">3 detected</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-400">Movement Stability</span><span className="text-green-400">Stable</span></div>
          </div>
        </div>
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5">
          <h4 className="text-white font-semibold text-sm mb-3">🌾 Produce Condition</h4>
          <div className="space-y-2">
            <div className="flex justify-between text-xs"><span className="text-gray-400">Estimated Quality</span><span className="text-green-400">Good</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-400">Handling Risk</span><span className="text-yellow-400">Moderate</span></div>
            <div className="flex justify-between text-xs"><span className="text-gray-400">Recommendation</span><span className="text-blue-400">Monitor closely</span></div>
          </div>
        </div>
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5">
